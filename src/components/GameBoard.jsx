...
function GameBoard({ config, onNewGame }) {
  ...
  const checkWin = () => {
    const puzzleLetters = new Set(
      config.puzzle.split("").filter((char) => /[A-Z]/.test(char))
    );

    const allRevealed = [...puzzleLetters].every((letter) =>
  ...
  const handleSpin = (result) => {
    ...
    if (result.type === "special") {
      ...
      setGamePhase("guess");
      setMessage(
        `${currentPlayer.name} spun $${result.value}. Choose a letter!`
      );
    }
  };
  ...
  const handleLetterGuess = (e) => {
    ...
    const count = config.puzzle.split("").filter((char) => char === letter)
      .length;
    ...
    if (count > 0) {
      ...
      setMessage(
        `Correct! ${count} ${letter}'s worth $${earned}. Spin again or solve!`
      );
      setGamePhase("spin");
    } else {
  ...
  const handleSolve = () => {
    ...
    if (solution && solution.toUpperCase() === config.puzzle) {
      ...
      setGameWon(true);
      setMessage(
        `${currentPlayer.name} solved it! Winner with $${updatedPlayers[currentPlayerIndex].score}!`
      );
      setGamePhase("won");

      const puzzleLetters = config.puzzle
        .split("")
    ...
    <div className="game-board">
      ...
      <div className="game-content">
        ...
        <div className="controls-section">
          ...
          <div className="round-score">Current Round: ${roundScore}</div>
